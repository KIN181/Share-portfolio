                    ...
                    @foreach ($user->following as $following_user)
                        <div class="row align-items-center mt-3">
                            <div class="col-auto">
                                <a href="{{ route('profile.show', $following_user->following->id) }}">
                                    @if ($following_user->following->avatar)
                                        <img src="{{ $following_user->following->avatar }}" alt="" class="rounded-circle avatar-sm">
                                    @else
                            ...
                            <div class="col ps-0 text-truncate">
                                <a href="{{ route('profile.show', $following_user->following->id) }}" class="text-decoration-none text-dark fw-bold">
                                    {{ $following_user->following->name }}
                                </a>
                            </div>
